...
export function SpeedtestStyleResults({
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      {/* Visually hidden summary for AT */}
      <p className="sr-only">
        {visible
          ? `Standard prompt ${fmtInt(standardTokens)} tokens, estimated ${fmtUsd(standardCost)}. Starbust prompt ${fmtInt(starbustTokens)} tokens, ${fmtUsd(starbustCost)}. Approximately ${pctSaved.toFixed(1)} percent fewer input tokens.`
      ...
      <div className="mt-2 grid grid-cols-2 gap-3 text-center">
        <div className="rounded-2xl bg-black/35 px-3 py-3 ring-1 ring-white/10">
          ...
          {pricePerMillion > 0 ? (
            <p className="mt-1 text-sm font-medium text-rose-200/80">
              {visible ? fmtUsd(standardCost) : '—'}{' '}
              <span className="text-slate-500">/ prompt</span>
            </p>
          ) : (
            <p className="mt-1 text-xs text-slate-600">Add price for USD</p>
          )}
        </div>
      ...
      {visible ? (
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-200 ring-1 ring-emerald-400/40">
            {pctSaved.toFixed(1)}% fewer tokens
          </span>
